...
export function HeroContent({ y, scale, opacity }: HeroContentProps) {
  ...
  return (
    <motion.div className="relative z-10 text-center px-6" style={{ y, scale, opacity }}>
      ...
      >
        {/* Mobile: Clean Grid of Skills */}
        <div className="md:hidden flex flex-wrap justify-center gap-3 px-4">
          {[
            { text: "Full-Stack", accent: false },
            { text: "Cloud Engineer", accent: true },
            { text: "DevOps", accent: false },
            { text: "AI/ML", accent: true },
            { text: "System Design", accent: false },
          ].map((skill, i) => (
            <motion.span
              key={skill.text}
              ...
              style={{
                ...
                color: skill.accent ? "var(--color-primary)" : "white",
                border: `1px solid ${skill.accent ? "rgba(204, 255, 0, 0.3)" : "rgba(255,255,255,0.1)"}`,
              }}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: false }}
              transition={{ delay: timing.subtitleDelay + 0.3 + i * 0.1 }}
            >
              {skill.text}
            </motion.span>
          ))}
        ...
        <div className="hidden md:block space-y-3 w-screen relative left-1/2 -translate-x-1/2 overflow-x-hidden">
          {/* Marquee 1 - Moving Left */}
          <div className="overflow-hidden">
            <div className="flex whitespace-nowrap items-center animate-marquee-left">
              {[...Array(4)].map((_, idx) => (
                <div key={idx} className="flex items-center flex-shrink-0">
                  <span
                    className="text-2xl lg:text-4xl px-4 flex-shrink-0"
                    style={{
                  ...
                  <span className="text-xl lg:text-3xl px-3 flex-shrink-0" style={{ color: "var(--color-dark-600)" }}>/</span>
                  <span className="text-2xl lg:text-4xl px-4 font-bold flex-shrink-0" style={{ color: "var(--color-primary)" }}>
                    Cloud Engineer
                  </span>
                  <span className="text-xl lg:text-3xl px-3 flex-shrink-0" style={{ color: "var(--color-dark-600)" }}>/</span>
                  <span
          ...
          <div className="overflow-hidden">
            <div className="flex whitespace-nowrap items-center animate-marquee-right">
              {[...Array(4)].map((_, idx) => (
                <div key={idx} className="flex items-center flex-shrink-0">
                  <span className="text-2xl lg:text-4xl px-4 font-bold flex-shrink-0" style={{ color: "var(--color-primary)" }}>
                    Web Development
